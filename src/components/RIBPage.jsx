...
export default function RIBPage({ user, navigate }) {
  ...
  const getBankInfo = (user) => {
    const bankCode = '30066';
    const swift = 'CMCIFRPPXXX';
    const bankName = '';
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 pb-24">
        ...
        <div className="bg-white rounded-xl shadow-lg overflow-hidden mb-6">
          {/* En-tête avec logo */}
          <div className=" bg-gray-600 to-teal-600 p-6 text-white">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-16 h-16  flex items-center justify-center p-2">
                  <img src="images/logo cic.jpeg" alt="CIC" className="w-full h-full object-contain" />
                </div>
                <div>
                  <h2 className="font-bold text-xl">{bankInfo.bankName}</h2>
                  <p className="text-sm opacity-90">Relevé d'Identité Bancaire</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <button 
            onClick={handleDownload}
            className="bg-gray-600 hover:bg-emerald-700 text-white py-4 rounded-xl font-medium transition flex items-center justify-center gap-2 shadow-sm"
          >
            <Download size={20} />
            Télécharger
          </button>
